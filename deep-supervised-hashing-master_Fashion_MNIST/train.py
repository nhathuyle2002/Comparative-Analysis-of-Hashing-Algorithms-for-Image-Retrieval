...
class MNISTPairDataset(Dataset):
    def __init__(self, data_root: str, transform=None, train=True):
        super().__init__()
        dt = FashionMNIST(root=data_root, train=train, transform=transform, download=True)
        self.dataset = Subset(dt, range(int(len(dt)*PERCENT_DATA_TAKING)))
...
if __name__ == '__main__':
    trainer = Trainer()
    trainer.train()
    saving_model_name = f'model_{PERCENT_DATA_TAKING}_{CODE_SIZE}_{EPOCH}_{ALPHA}_{MARGIN}_{time.strftime("%Y%m%d-%H%M%S")}.pt'
    torch.save(model, saving_model_name)